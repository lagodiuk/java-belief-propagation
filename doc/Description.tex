...
\State \textbf{preliminary remarks:}
...
In given representation -- each of compound nodes corresponds to unique chemical compound. And, as far as each chemical compound must be of some of compound types -- this means, that each \emph{compound node is implicitly belongs to corresponding compound type}. The same statement (about implicit correspondence to reaction type) is also true for \emph{reaction nodes}.

Taking into account the fact about implicit correspondence between graph nodes and types of reactions and compounds -- we might conclude, that type of chemical compound is defined only by types of chemical reactions, in which given compound participate.
