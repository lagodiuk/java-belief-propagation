...
\begin{tikzpicture}[
    ...
    reaction node/.style={circle,thick,double distance=0.4mm,fill=green!20,minimum width=0.7cm,draw}]
...
\begin{equation}
P(x) = {1 \over Z} \left( \prod_{(i,j) \in A_R} \psi_{reagent}(x_i, x_j) \right) \left( \prod_{(m,k) \in A_P} \psi_{product}(x_m, x_k) \right)
\end{equation}
where:
\begin{itemize}
    \setlength \itemsep{0em}
...
\begin{tikzpicture}[
    ...
    compound node/.style={circle,fill=blue!20,minimum width=1.7cm,draw},
    reaction node/.style={circle,thick,double distance=0.4mm,fill=green!20,minimum width=0.7cm,draw},
    observed node/.style={circle,fill=black,inner sep=.2cm, node distance = 1.7cm,outer sep=0}]
  ...
  \path[-]
    (NaOH) 
         edge node[left] {} (Reaction1)
         edge node[right] {} (Reaction2)
         edge node[right] {} (ObservedNaOH)
    (H2SO4) 
         edge node[right] {} (Reaction1)
         edge node[right] {} (ObservedH2SO4)
    (Reaction1) 
         edge node [right] {} (Na2SO4)
         edge node [left] {} (H2O)
         edge node[right] {} (ObservedReaction1)
    (SO3) 
         edge node [left] {} (Reaction2)
         edge node[right] {} (ObservedSO3)
    (Reaction2) 
         edge node [left] {} (Na2SO4)
         edge [bend right = 90] node [right] {} (H2O)
         edge node[right] {} (ObservedReaction2)
    (H2O) 
         edge node [left] {} (Reaction3)
         edge node[right] {} (ObservedH2O)
    (Na2O) 
         edge node [right] {} (Reaction3)
         edge node[right] {} (ObservedNa2O)
    (Reaction3) 
         edge [bend right = 90] node [right] {} (NaOH)
         edge node[right] {} (ObservedReaction3)
    (Na2SO4)
         edge node[right] {} (ObservedNa2SO4);
