...
\begin{equation}
\begin{split}
   & O\left(\ |S(X_1)| \times |S(X_2)| \times \dots \times |S(X_{k-1})| \times |S(X_{k+1})| \times \dots \times |S(X_n)| \ \right) = O(\alpha^{n - 1}) = O(\alpha^n), \\
   & \forall k \in I \cup J,\\
   & \text{$\alpha$ -- is just some constant}, \\
   & \text{$n$ -- is number of vertices in graph $G$}
\end{split}
\end{equation}
...
Taking into account \Cref{eq:marginal_probabilities_one_node_b,eq:marginal_probabilities_two_nodes_b}: we can substitute summations of probability mass function -- to marginal probabilities:
\begin{equation}
\begin{split}
...
Again, taking into account \Cref{eq:marginal_probabilities_one_node_b,eq:marginal_probabilities_two_nodes_b}: we can substitute summations of probability mass function -- to marginal probabilities:
\begin{equation}
\begin{split}
...
\begin{split}
   b_{ij}(x_i, x_j) & = Z_{ij} \times \left( \prod_{k \in N(i) \setminus j} m_{k \rightarrow i}(x_i) \right) \times \phi_i(x_i) \times \psi_{ij}(x_i, x_j) \times \phi_j(x_j) \times \left( \prod_{k \in N(j) \setminus i} m_{k \rightarrow j}(x_j) \right), \\ 
                          & \text{where } Z_{ij} \text{ is such that } \sum_{x_i} \sum_{x_j} b_{ij}(x_i,x_j) = 1 \\
   b_i(x_i)           & = Z_i \times \phi_i(x_i) \times \left( \prod_{k \in N(i)} m_{k \rightarrow i}(x_i) \right), \\
                          & \text{where } Z_i \text{ is such that } \sum_{x_i} b_i(x_i) = 1 \\
   b_j(x_j)           & = Z_j \times \phi_j(x_j) \times \left( \prod_{k \in N(j)} m_{k \rightarrow j}(x_j) \right), \\
                          & \text{where } Z_j \text{ is such that } \sum_{x_j} b_j(x_j) = 1
\end{split}
\end{empheq}
...
\bibitem{wikipedia_markov_random_field}
