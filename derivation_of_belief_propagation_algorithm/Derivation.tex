...
Keeping in mind Equation \eqref{eq:probability_sum}, lets consider following equalities for marginal probabilities (which will be useful during the further steps of derivation):
...
\begin{split}
& \sum_{(i,j)} f_i(x_i) \equiv \sum_{i} \left( q_i \times f_i(x_i) \right) \\
& \sum_{(i,j)} f_j(x_j) \equiv \sum_{j} \left( q_j \times f_j(x_j) \right)
\end{split}
\end{equation}
