...
Lets consider single-node summation over all edges: where $q_i$ -- number of edges, which connected to node $i$ (the same notations is for node $j$ -- $q_j$);
\begin{equation} \label{eq:single_node_sum_over_edges}
\begin{split}
& \sum_{(i,j)} p_i(x_i) = \sum_{i} \left( q_i \times p_i(x_i) \right) \\
& \sum_{(i,j)} p_j(x_j) = \sum_{j} \left( q_j \times p_j(x_j) \right)
\end{split}
\end{equation}
...
\begin{equation} \label{eq:single_node_sum_over_edges_b}
\begin{split}
& \sum_{(i,j)} b_i(x_i) = \sum_{i} \left( q_i \times b_i(x_i) \right) \\
& \sum_{(i,j)} b_j(x_j) = \sum_{j} \left( q_j \times b_j(x_j) \right)
\end{split}
\end{equation}
...
\begin{split}
   ...
   & = \left( \sum_{(i, j)} \sum_{x_i} \sum_{x_j} b_{ij}(x_i, x_j) \times E_{ij}(x_i, x_j) \right) - \left( \sum_{(i, j)} \sum_{x_i} \sum_{x_j} b_{ij}(x_i, x_j) \times E_i(x_i) \right) - \\
   & -\left( \sum_{(i, j)} \sum_{x_i} \sum_{x_j} b_{ij}(x_i, x_j) \times E_j(x_j) \right) + \left( \sum_{i} \sum_{x_i} b_i(x_i) \times E_i(x_i) \right) + \left( \sum_{j} \sum_{x_j} b_j(x_j) \times E_j(x_j) \right)
\end{split}
\end{equation}
